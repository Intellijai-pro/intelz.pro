    ...
    <div class="card-header">
      <x-backend.section-header>
        ...
        <x-slot name="toolbar">
          ...
          @hasPermission('add_customtemplate')
...
<div data-render="app">
  <customtemplate-offcanvas default-image="{{default_feature_image()}}" create-title="{{ __('Create') }} {{ __($module_title) }}"
    edit-title="{{ __('Edit') }} {{ __($module_title) }}" :customefield="{{ json_encode($customefield) }}">
  </customtemplate-offcanvas>
...
@push('after-scripts')
<script src="{{ mix('modules/customtemplate/script.js') }}"></script>
...
<script type="text/javascript" defer>
  const columns = [{
      ...
      },
      {
        ...
        orderable: false,
        searchable: false,
        title: "{{ __('customtemplate.lbl_image') }}"
      },
      {
        data: 'template_name',
        name: 'template_name',
        title: "{{ __('customtemplate.lbl_name') }}"
      },
      {
        data: 'category_id',
        name: 'category_id',
        title: "{{ __('customtemplate.lbl_category') }}"
      },
    ...
    document.addEventListener('DOMContentLoaded', (event) => {
      initDatatable({
        url:'{{ route("backend.$module_name.index_data") }}',             // url: '{{ route("backend.$module_name.index_data") }}',
        finalColumns,
      })
    })
